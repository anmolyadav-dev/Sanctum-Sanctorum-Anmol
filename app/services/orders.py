...
def calculate_discount_percent(member: Member, total_quantity: int) -> int:
    ...
    tier_pct = TIER_DISCOUNT_PERCENT.get(member.tier, 0)
    bulk_pct = BULK_DISCOUNT_PERCENT if total_quantity >= BULK_QUANTITY_THRESHOLD else 0
    return tier_pct + bulk_pct
...
def create_order(db: Session, data: OrderCreate, now: datetime) -> Order:
    ...
    if any(books_by_id[item.book_id].restricted for item in data.items):
